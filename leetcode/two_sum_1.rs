...
fn two_sum(nums: Vec<i32>, target: i32) -> Vec<i32> {
    ...
    while left < right {
        let sum = value_index_pairs[left].0 + value_index_pairs[right].0;
        if sum == target {
            // Am găsit o soluție. Returnăm imediat indicii originali.
            // Ordinea indicilor (cel mai mic primul) nu e garantată aici,
            // dar poți folosi min/max dacă e necesar.
            return vec![value_index_pairs[left].1 as i32, value_index_pairs[right].1 as i32];
        } else if sum > target {
            right -= 1;
        } else {
            left += 1;
    ...
    // Dacă nu s-a găsit nicio soluție, returnează un vector gol.
    Vec::new()
}
